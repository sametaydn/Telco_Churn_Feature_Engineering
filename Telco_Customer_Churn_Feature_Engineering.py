...
df = pd.read_csv(r"C:\Users\Sade\Desktop\VBO\python\Modül_5_Özellik_Mühendisliği\Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
...
def check_df(dataframe, head=5):
    ...
    print("##################### Quantiles #####################")
    print(dataframe.quantile([0, 0.05, 0.50, 0.75,0.95,0.97,0.98, 0.99, 1]).T)
...
def cat_summary(dataframe, col_name, plot=False):
    print(pd.DataFrame({col_name: dataframe[col_name].value_counts(),
                        "Ratio": 100 * dataframe[col_name].value_counts() / len(dataframe)})) #len(df)
    print("##########################################")
    if plot:
...
#df.groupby(cat_cols[0])["Churn"].value_counts()

def target_summary_with_num(dataframe, target, numerical_col):
    print(numerical_col)
    print(pd.DataFrame({"TARGET_MEAN": dataframe.groupby(target)[numerical_col].mean()}), end="\n\n\n")
for col in num_cols:
    target_summary_with_num(df, "Churn", col)
...
def target_summary_with_cat(dataframe, target, categorical_col):
    """
    target_summary_with_cat shows that mean,median and count values according to target variable.
...
plot_importance(catboost_model, X_test)
